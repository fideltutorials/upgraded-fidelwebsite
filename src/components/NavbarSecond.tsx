...
export default function NavbarSecond() {
  ...
  return (
    <>
      ...
      >
        <div
          className={`absolute top-4 right-4 bottom-4 left-4 rounded-3xl bg-brand-paper shadow-2xl p-6 overflow-y-auto flex flex-col transition-transform duration-300 ${mobileMenuOpen ? "scale-100" : "scale-95 pointer-events-none opacity-0"
            }`}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between pb-4 border-b border-brand-rule">
          ...
          <nav className="flex-1 py-4">
            <ul>
              {navItems.map((item, idx) => (
                <li key={idx} className="border-b border-brand-rule/30 py-2">
                  {item.href ? (
                    <a href={item.href} onClick={() => setMobileMenuOpen(false)} className="block py-2 text-base font-semibold text-brand-ink">
                      {item.label}
                    </a>
                  ) : (
                    <div className="flex flex-col gap-2">
                      <span className="block py-2 text-base font-semibold text-brand-muted">{item.label}</span>
                      <div className="pl-3 border-l border-brand-secondary flex flex-col gap-2.5">
                        {item.submenu?.map((sub, sIdx) => (
                          <a key={sIdx} href={sub.href} onClick={() => setMobileMenuOpen(false)} className="block">
                            <span className="block text-sm font-bold text-brand-ink">{sub.label}</span>
          ...
          <a href="/book" onClick={() => setMobileMenuOpen(false)} className="w-full text-center bg-brand-primary text-brand-paper py-3 rounded-full font-bold text-sm shadow-md mt-4">
            Book a Free Trial
          </a>
        </div>
      </div>
