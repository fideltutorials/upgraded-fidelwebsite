              ...
              return (
                <div key={program.id}>
                  <div 
                    ...
                    id={program.slug}
                  >
                    <div className={`lg:col-span-5 flex justify-center lg:justify-start ${!isEven ? 'order-1 lg:order-2 lg:justify-end' : ''}`}>
                      <a href={`/programs/${program.slug}`} className="group block hover:scale-[1.02] transition-transform duration-300">
                    ...
                    <div className={`lg:col-span-7 flex flex-col items-start ${!isEven ? 'order-2 lg:order-1' : ''}`}>
                      ...
                      {benefitsArray.length > 0 && (
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-brand-ink font-semibold mb-6">
                          {benefitsArray.map((benefit, bIdx) => (
                            <li key={bIdx} className="flex items-center gap-2">✓ {benefit}</li>
                          ))}
                        </ul>
                      )}
                      <a href={`/programs/${program.slug}`} className="bg-brand-primary text-brand-paper hover:bg-brand-primary-deep px-6 py-3 rounded-full font-semibold transition-all">
                        Learn More →
                      </a>
                    </div>
                  </div>
                  {idx < dbPrograms.length - 1 && <hr className="border-brand-rule my-16" />}
                </div>
              );
            })
          ) : (
            <div className="text-center py-20 bg-brand-cream-warm/20 rounded-2xl border border-brand-rule border-dashed">
              <p className="text-brand-muted text-sm">No programs configured in the database.</p>
            </div>
          )}
