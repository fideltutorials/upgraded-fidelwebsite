...
export default function Tutors() {
  ...
  useEffect(() => {
    ...
    if (selectedGrade !== "All") {
      filtered = filtered.filter(
        (t) => t.grades.includes(selectedGrade) || t.grades.includes("All"),
      );
    }
  ...
  const allSpecialties = [
    "All",
    ...Array.from(new Set(tutorsList.flatMap((t) => t.specialties))),
  ];

  // Extract all unique grades
  const allGrades = [
    "All",
    ...Array.from(new Set(tutorsList.flatMap((t) => t.grades))),
  ];
    ...
    >
      ...
      <section className="py-16 md:py-24 bg-brand-cream-warm/40 border-b border-brand-rule">
        <div className="max-w-[1200px] mx-auto px-6 text-center">
          ...
          <p className="text-brand-muted text-base sm:text-lg leading-relaxed max-w-2xl mx-auto">
            Our network consists of vetted, high-performing tutors trained in
            our results-driven methods. Filter below by subject and level to
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between p-6 border border-brand-rule bg-brand-cream-warm/10 rounded-2xl mb-12 shadow-sm">
            ...
            <div className="flex flex-wrap gap-4 items-center w-full md:w-auto justify-end">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-brand-muted/80 uppercase tracking-wide whitespace-nowrap">
                  Specialty:
                </span>
                <select
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-brand-cream-warm/20 rounded-2xl border border-brand-rule border-dashed">
              ...
              <p className="text-brand-muted text-sm max-w-sm mx-auto mb-6">
                We couldn't find any tutors matching "{searchQuery}" with your
                chosen subject filters.
              </p>
              <button
                onClick={handleResetFilters}
              ...
              {filteredTutors.map((tutor) => (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <p className="text-brand-muted text-xs leading-relaxed mb-6 flex-1">
                      {tutor.bio}
                    </p>
      ...
      <section className="py-20 bg-brand-cream-warm/40 border-t border-brand-rule">
        <div className="max-w-[800px] mx-auto px-6 text-center">
          ...
          <h2 className="font-serif text-3xl md:text-4xl font-semibold text-brand-ink mb-4">
            Apply to Teach with Us
          </h2>
          <p className="text-brand-muted text-base leading-relaxed mb-8">
            Are you a passionate educator or subject matter expert looking to
            make a difference? We are always expanding our network of vetted,
            trained, and certified tutors in Addis Ababa and online.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <a
              href="mailto:careers@fideltutorial.com"
              className="bg-brand-primary text-brand-paper hover:bg-brand-primary-deep px-8 py-4 rounded-full font-bold transition-all hover:-translate-y-0.5 shadow-md"
            >
              Submit Tutor Application
            </a>
            <a
              href="/contact"
              className="border border-brand-rule hover:bg-brand-cream-warm text-brand-ink px-8 py-4 rounded-full font-bold transition-all"
            >
